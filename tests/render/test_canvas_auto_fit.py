...
def test_branch_axis_edge_returns_zero_for_empty_layout() -> None:
    ...
    # --- arrange ----------------------
    layout, theme = _layout_and_theme(build_jsonl({"op": "branch", "name": "main"}))
...
def test_commit_axis_lower_uses_pill_width_in_horizontal_orientation() -> None:
    ...
    short_jsonl = build_jsonl(
        {"op": "theme", "orientation": "lr"},
        {"op": "branch", "name": "m"},
        {"op": "commit", "branch": "m", "id": "c1", "msg": "x"},
    )
    long_jsonl = build_jsonl(
        {"op": "theme", "orientation": "lr"},
        {"op": "branch", "name": long_name},
        {"op": "commit", "branch": long_name, "id": "c1", "msg": "x"},
    )
