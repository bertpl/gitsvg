...
def test_default_is_half_a_font_size() -> None:
    ...
    # Resolved px = ratio * label_font_size.
    assert theme.table_cell_padding_x == 0.5 * theme.label_font_size
...
def test_override_resolves_to_pixels() -> None:
    ...
    # --- assert -----------------------
    assert theme.table_cell_padding_x == theme.label_font_size


def test_zero_is_allowed() -> None:
    """Zero padding is legal (cells abut the column edge); only negatives are rejected."""
    # --- arrange / act / assert -------
    assert DefaultTheme.build({"table_cell_padding_x_in_font_sizes": 0.0}).table_cell_padding_x == 0.0
