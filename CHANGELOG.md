...
- New `gitsvg schema themes` (list registered named themes) and `gitsvg schema theme <name>` (print one theme's resolved field values).
- New shipped example `examples/10_named_themes.svg` — a tiled preview showing the `dark` and `compact` themes side-by-side; generated by `scripts/build_themed_preview.py`.
