...
| Command | Purpose |
|---------|---------|
| `gitsvg render <input> -o <output>` | Render a `.gitsvg.jsonl` file to SVG. Pass a directory at both ends to walk the input tree and write mirrored `.svg` outputs under the output directory. `--small=N` selects minification level 0-3; bare `--small` is level 2 (lossless structural compression). |
